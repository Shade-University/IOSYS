...
typedef struct {
	...
	sem_t	sync_patient; 	// Semaphore to synchronize patients
} people_data_t;
...
sem_t sync_nurse; // semaphore to synchronize nurse
bool sync_nurse_initialized = false; // to keep track of initialization of nurse semaphore
pthread_barrier_t sync_vaccination; // barrier to synchronize start of vaccination
bool sync_vaccination_initialized = false; // to keep track of initialization of vaccination barrier
pthread_mutex_t cs_choosing = PTHREAD_MUTEX_INITIALIZER; // mutex to fix critical section with default initialization
bool cs_choosing_initialized = true;
...
void release_resources(void)
{
	...
	if(sync_nurse_initialized) {
		if(sem_destroy(&sync_nurse))
			perror("sync_destroy");
		sync_nurse_initialized = false;
	} // Release nurse semaphore

	if(sync_vaccination_initialized) {
		if (pthread_barrier_destroy(&sync_vaccination))
			perror("pthread_barrier_destroy");
		sync_vaccination_initialized = false;
	} // Release vaccination barrier

	if(cs_choosing_initialized) {
		if(pthread_mutex_destroy(&cs_choosing))
			perror("pthread_mutex_destroy");
		cs_choosing_initialized = false;
	} // Release choosing mutex

	int people = 0;
	for (people = 0; people < MAX_PEOPLE; ++people) {
		if(sem_destroy(&vaccination_list[people].sync_patient))
			perror("sync_destroy");
	} //Release all patient semaphores
}
...
{
	switch (pthread_barrier_wait(&sync_vaccination)) {
...
#define	thread_id	(*(int *)arg)	// id of the thread, index to people_data[]
	...
	if(pthread_mutex_lock(&cs_choosing)) {
	...
	if (pos < positions) {	// a free spot found
		...
		if(sem_init(&vaccination_list[pos].sync_patient, 0, 0)) {
			perror("sem_init");
			exit(EXIT_FAILURE);
		} // Initialize semaphore for patient
	}

	if(pthread_mutex_unlock(&cs_choosing)) {
	...
	// vaccination
	if(sem_wait(&vaccination_list[pos].sync_patient)) {
		perror("sem_wait");
		exit(EXIT_FAILURE);
	} // wait for invitation
	printf(C_NAME NAME_FMT C_NORMAL " is going to take a vaccine\n", people_data[thread_id].name);
	if(sem_post(&sync_nurse)) {
		perror("sem_post");
		exit(EXIT_FAILURE);
	} // Notify nurse
	if(sem_wait(&vaccination_list[pos].sync_patient)) {
...
void *nurse(void *arg)
{
	...
	for (pos = 0; pos < positions && vaccination_list[pos].group > 0; ++pos) {
		// invite a patient
		if(sem_post(&vaccination_list[pos].sync_patient)) {
			perror("sem_post");
			exit(EXIT_FAILURE);
		}
		if(sem_wait(&sync_nurse)) {
			perror("sem_wait");
			exit(EXIT_FAILURE);
		} //Wait for patient
...
int main(int argc, char *argv[])
{
	...
	// print id
	printf("Modified by: st55448 Vondra Tomáš\n"); 
	// initialize resources
	if(sem_init(&sync_nurse, 0, 0)) {
		perror("sem_init");
		exit(EXIT_FAILURE);
	}
	sync_nurse_initialized = true; // nurse semaphore init

	
	if (pthread_barrier_init(&sync_vaccination, NULL, people + 1)) {
		perror("pthread_barrier_init");
		exit(EXIT_FAILURE);
	}
	sync_vaccination_initialized = true; // vaccination barrier init
